...
def update():
    ...
    if request.args.get('two'):
        ...
        """ scheduling an update. update_name and update_time will be set via the interface form inputs.
        param 3 = news search kw, param 4 = city for covid data, param 5 = nation of covid data """

        schedule_covid_updates(update_name, update_interval, config['interface_data']['news_kw'],
        config['interface_data']['area_name'], config['interface_data']['nation'])
...
def schedule_covid_updates(update_name: str, update_interval: str, news_kw: str, area_name='Exeter', nation='England'):
